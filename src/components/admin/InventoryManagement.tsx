...
import { getMedicineInventory, addMedicineToInventory, updateMedicineInInventory } from '../../services/api';
...
const InventoryManagement: React.FC = () => {
    ...
    const handleFormSubmit = async (item: Omit<MedicineInventory, 'medicineId'> | MedicineInventory) => {
        if ('id' in item && item.id) {
            await updateMedicineInInventory(item.id, item);
        } else {
            ...
            <div className="bg-white rounded-xl shadow p-6">
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-gray-500">
                        ...
                        <tbody>
                            {inventory.map(item => (
                                <tr key={item.id} className={`border-b ${item.quantity < item.threshold ? 'bg-yellow-50' : 'bg-white'} hover:bg-gray-50`}>

...
import { getAppointmentsForStudent, getStaff, bookAppointment as apiBookAppointment, cancelAppointment as apiCancelAppointment, getAvailableTimeSlots } from '../../services/api';
...
const StudentAppointments: React.FC = () => {
    ...
    const handleCancelAppointment = async (docId: string) => {
        if (window.confirm('Are you sure you want to cancel this appointment?')) {
            await apiCancelAppointment(docId);
            fetchData();
        }
    };
                ...
                {upcomingAppointments.length > 0 ? (
                    <div className="space-y-4">
                        {upcomingAppointments.map(app => (
                            <div key={app.id} className="p-4 border rounded-lg flex flex-col md:flex-row justify-between items-start md:items-center">
                                ...
                                <div className="mt-4 md:mt-0 flex flex-col md:items-end space-y-2 w-full md:w-auto">
                                    ...
                                    {(app.status === AppointmentStatus.Pending || app.status === AppointmentStatus.Confirmed) && (
                                        <button onClick={() => handleCancelAppointment(app.id!)} className="text-xs text-brand-danger hover:underline">Cancel</button>
                                    )}
                                </div>
                            </div>
                        ))}

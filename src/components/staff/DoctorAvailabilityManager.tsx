...
import { getStaff, getDoctorAvailabilities, addDoctorAvailability, updateDoctorAvailability, deleteDoctorAvailability } from '../../services/api';
...
const DoctorAvailabilityManager: React.FC = () => {
    ...
    const handleFormSubmit = async (slot: Omit<DoctorAvailability, 'availabilityId'> | DoctorAvailability) => {
        if ('id' in slot && slot.id) {
            await updateDoctorAvailability(slot.id, slot);
        } else {
            await addDoctorAvailability(slot as Omit<DoctorAvailability, 'availabilityId'>);
        }
        setShowForm(false);
        setSlotToEdit(null);
    ...
    const handleDelete = async (docId: string) => {
        if(window.confirm("Are you sure you want to delete this slot? This cannot be undone.")) {
            await deleteDoctorAvailability(docId);
            if (selectedDoctorId) fetchData(selectedDoctorId);
        }
    };
                                ...
                                <tr key={slot.id} className="bg-white border-b hover:bg-gray-50">
                                    ...
                                    {!isReadOnly && (
                                        <td className="px-6 py-4 flex items-center gap-4">
                                            ...
                                            <button onClick={() => handleDelete(slot.id!)} className="text-brand-danger hover:text-red-700"><Trash2 size={18} /></button>
                                        </td>
                                    )}
                                </tr>
                            ))}

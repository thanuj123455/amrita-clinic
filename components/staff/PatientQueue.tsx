...
// Fix: Imported VisitStatus to use the enum member for type safety.
import { Appointment, Student, Staff, AppointmentStatus, PatientVisit, Vital, Prescription, VisitStatus } from '../../types';
import { getAppointments, getStudents, createPatientVisit, updateAppointmentStatus, getStaff, getMedicineInventory, createPrescription as apiCreatePrescription, generateVisitSummaryPDF, generatePrescriptionPDF } from '../../services/mockApi';
...
const PatientVisitModal: React.FC<{ visit: PatientVisit, student: Student, onSave: (visit: PatientVisit, prescriptions: Omit<Prescription, 'prescriptionId'>[]) => void, onClose: () => void }> = ({ visit, student, onSave, onClose }) => {
    ...
    const handleSaveAndClose = async () => {
        // Fix: Used VisitStatus.Closed enum member instead of a string literal to match the type definition.
...
const PatientQueue: React.FC = () => {
    ...
    const [activeVisit, setActiveVisit] = useState<{visit: PatientVisit, student: Student} | null>(null);
    ...
    const handleCheckIn = async (appointment: Appointment & {student: Student}) => {
        if(user && 'staffId' in user) {
            ...
            setActiveVisit({visit: newVisit, student: appointment.student});
        }
    };
    ...
    const handleSaveVisit = async (visit: PatientVisit, prescriptions: Omit<Prescription, 'prescriptionId'>[]) => {
        ...
        await updateAppointmentStatus(activeVisit!.visit.studentId, AppointmentStatus.Completed);
